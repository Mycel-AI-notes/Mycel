...
/// Destructive counterpart to `kb_deinit`: remove the KB *and* its backing
/// data (`.db.json` + `index.md`). The user's `.md` notes are left alone —
/// they were the user's content before the KB existed and remain so after.
///
/// `confirm_name` must equal the directory basename (e.g. for `books/read`
/// it must be `read`). The match is checked in the backend as a final
/// safety belt so a buggy or malicious caller can't blow away a KB without
/// having seen and typed the name.
#[tauri::command]
pub async fn kb_delete(
    ...
    let basename = dir_rel.rsplit('/').next().unwrap_or(&dir_rel);
    if confirm_name.trim() != basename {
        return Err(format!(
            "Confirmation mismatch: expected '{basename}', got '{}'.",
            confirm_name.trim()
        ));
    }
